...
export class AddCommunityModalComponent implements OnInit, AfterViewInit {
  @Input() title: string | undefined = 'Create Visionary';
        ...
        this.communityService.createCommunity(this.communityForm.value).subscribe(
          {
            next: (res: any) => {
              ...
              if (!res.error) {
                ...
                this.toastService.success('Your Local Visionary will be approved within 24 hours!');
                this.activeModal.close('success');
              }
            },
            error:
              (err) => {
                this.toastService.danger('Please change visionary name. this visionary name already in use.');
                this.spinner.hide();
              }
          });
      } else {
      ...
      this.communityService.editCommunity(this.communityForm.value, this.data.Id).subscribe(
        {
          next: (res: any) => {
            ...
            if (!res.error) {
              this.submitted = true;
              // this.createCommunityAdmin(res.data);
              this.toastService.success('Your Local Visionary edit successfully!');
              this.activeModal.close('success');
            }
          },
          error:
